...
class BinanceBatchDownloader:
    ...
    def __init__(self, pairs: List[str], download_path: Union[str, Path], 
                 start_date: str, overwrite_mode: bool = False, enable_logging: bool = True):
        """
        Args:
        ...
        # 初始化失败记录字典
        self.failed_downloads: Dict[str, List[str]] = {pair: [] for pair in pairs}
    ...
    def _get_existing_dates(self, pair_path: Path) -> List[str]:
        ...
        if not pair_path.exists():
            return []
        return [str(file_path)[-14:-4] for file_path in pair_path.iterdir() 
    ...
    def download_all(self) -> None:
        ...
        self._print_status(f"开始批量下载，交易对: {self.pairs}, 日期范围: {self.start_date} 到今天")
